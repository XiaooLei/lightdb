...
namespace lightdb{
    ...
    Status LightDB::StrSet(const std::string& key, const std::string& value){
        Status s;
    ...
    Status LightDB::StrSetNx(const std::string& key, const std::string& value, bool& suc){
        Status s;
        bool exist;
        exist = StrExist(key);
        if(exist){
    ...
    Status LightDB::GetSet(const std::string& key, std::string& oldValue, const std::string& newValue, bool& suc){
        Status s;
    ...
    Status LightDB::Append(const std::string& key, const std::string& value, int& length){
        Status s;
        s = CheckKeyValue(key, value);
        if(!s.ok()){
            return s;
    ...
    Status LightDB::Remove(const std::string& key, bool& suc){
    ...
    Status LightDB::Expire(const std::string& key, uint64_t duration, bool& suc){
        Status s;
        std::string value;
        s = getVal(key, value, suc);
        if(!s.ok()){
    ...
    // TTL Time to live.
    int64_t LightDB::TTL(const std::string& key){
        Status s;
    ...
    bool LightDB::StrExist(const std::string& key){
        std::lock_guard<std::mutex> lockGuard(this->strIdx.mtx);
        Status s;
        s = CheckKeyValue(key, "");
        if(!s.ok()){
    ...
    Status LightDB::getVal(const std::string& key, std::string& value, bool& suc){
        std::lock_guard<std::mutex> lockGuard(this->strIdx.mtx);
        Status s;
        Indexer idx;
